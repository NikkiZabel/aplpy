...
class FITSFigure(Layers, Regions):
    ...
    @auto_refresh
    def make_square(self, pad_value=0):
        """
        ...
        shape_diff = np.max(self._data.shape) - np.min(self._data.shape)
        square_img = np.ones((np.max(self._data.shape), np.max(self._data.shape))) * pad_value

        if self._data.shape[0] > self._data.shape[1]:
            self._header['CRPIX1'] += int(shape_diff / 2)
            square_img[:, int(shape_diff / 2):int(shape_diff / 2 + self._data.shape[1])] = self._data
        else:
            self._header['CRPIX2'] += int(shape_diff / 2)
            square_img[int(shape_diff / 2):int(shape_diff / 2 + self._data.shape[0]), :] = self._data

    @auto_refresh
    def cut_empty_rows(self, parallel_hdu=None, empty_value=0, pad=0, centre=None, centre_frame='sky'):
        """
        ...
        # Find empty rows
        empty_x = np.all(self._data == empty_value, axis=1)
        ...
        if parallel_hdu:
            return parallel_hdu_trimmed, centre
        else:
            return centre

    @auto_refresh
    def cut_empty_columns(self, parallel_hdu=None, empty_value=0, pad=0, centre=None, centre_frame='sky'):
        """
        ...
        # Find empty columns
        empty_y = np.all(self._data == empty_value, axis=0)

        # If the number of empty columns is larger than the desired pad, cut away rows and leave as many as the pad
    ...
    @auto_refresh
    def centre_data(self, centre, frame='sky', parallel_hdu=None, pad_value=0):
        """
        ...
        shift_x = int(np.round(centre_pix[1] - self._data.shape[0] / 2))
        shift_y = int(np.round(centre_pix[0] - self._data.shape[1] / 2))

        # Pad the image with twice the amount it has to shift, so that the new centre overlaps with the coordinates
        if shift_x > 0:
            temp = np.ones((self._data.shape[0] + shift_x * 2, self._data.shape[1])) * pad_value
            temp[0:self._data.shape[0], :] = self._data
            if parallel_hdu:
                temp_parallel = np.ones(
                    (parallel_hdu_centred.shape[0] + shift_x * 2, parallel_hdu_centred.shape[1])) * pad_value
                temp_parallel[0:parallel_hdu_centred.shape[0], :] = parallel_hdu_centred.data
        elif shift_x < 0:
            temp = np.ones((self._data.shape[0] + abs(shift_x) * 2, self._data.shape[1])) * pad_value
            temp[temp.shape[0] - self._data.shape[0]:temp.shape[0], :] = self._data
            if parallel_hdu:
                temp_parallel = np.ones(
                    (parallel_hdu_centred.shape[0] + abs(shift_x) * 2, parallel_hdu_centred.shape[1])) * pad_value
                temp_parallel[temp_parallel.shape[0] - parallel_hdu_centred.shape[0]:temp_parallel.shape[0],
                :] = parallel_hdu_centred.data
        else:
            temp = self._data
            if parallel_hdu:
        ...
        if shift_y > 0:
            ...
            if parallel_hdu:
                parallelhdu_new = np.ones(
                    (temp_parallel.shape[0], temp_parallel.shape[1] + abs(shift_y) * 2)) * pad_value
                parallelhdu_new[:,
    ...
    def trim_image(self, parallel_hdu=None, empty_value=0, pad=0, centre=None, centre_frame='sky', square=False,
        ...
        if np.any(centre):
            if not len(centre) == 2:
                raise TypeError(
                    'For "centre", please provide a tuple, list, or ndarray of length 2 containing either pixel coordinates'
        ...
        if parallel_hdu:
            parallel_hdu, centre = self.cut_empty_rows(parallel_hdu=parallel_hdu, empty_value=empty_value,
                                                            pad=pad, centre=np.array(centre), centre_frame=centre_frame)
            parallel_hdu, centre = self.cut_empty_columns(parallel_hdu=parallel_hdu, empty_value=empty_value,
                                                          pad=pad, centre=np.array(centre), centre_frame=centre_frame)
            if np.any(centre):
                parallel_hdu = self.centre_data(centre=centre, frame=centre_frame, parallel_hdu=parallel_hdu,
                                                pad_value=pad_value)
            if square:
                self.make_square(pad_value=pad_value)
                parallel_hdu = self.make_square(parallel_hdu, pad_value=pad_value)
            return parallel_hdu
        else:
            centre = self.cut_empty_rows(parallel_hdu=parallel_hdu, empty_value=empty_value, pad=pad,
                                         centre=np.array(centre), centre_frame=centre_frame)
            centre = self.cut_empty_columns(parallel_hdu=parallel_hdu, empty_value=empty_value, pad=pad,
                                            centre=np.array(centre), centre_frame=centre_frame)
            if np.any(centre):
                self.centre_data(centre=centre, frame=centre_frame, parallel_hdu=parallel_hdu,
                                  pad_value=pad_value)
            if square:
                self.make_square(pad_value=pad_value)
